...
```mermaid
%%{init: {'themeVariables': { 'edgeInterpolate': 'linear' }}}%%
graph TD
    %% User Interface Layer
    subgraph UI ["User Interface"]
        style UI fill:#ffffeb,stroke:#333,stroke-width:1px
        direction LR
        DW["Document Workspace<br/>(Upload & Analyze)"]
        IC["IPC v/s BNS<br/>Comparator"]
        RD["Research Dashboard<br/>(Chat & Search)"]
        CP["Case/Law Portal<br/>(Find laws and past cases)"]
    end

    %% Document Analysis Layer
    subgraph DA ["Document Analysis"]
        PR["Private Retrieval<br/>(MMR Logic)"]
        RTS["Recursive Text Splitter"]
    end

    %% Legal Research Engine Layer
    subgraph LRE ["Legal Research Engine"]
        style LRE fill:#fff5f2,stroke:#333
        ALA["AI Legal Agent<br/>(LLM Reasoning)"]
        DE["Diffing Engine"]
        
        subgraph HRS ["Hybrid Retrieval System"]
            style HRS fill:#f3e5f5,stroke:#333
            BM25["BM25 Keyword Search<br/>(Syntactical Match)"]
            VS["Vector Search<br/>(Semantic Match)"]
            RRF["RRF Rank Fusion<br/>(Weighted Merger)"]
    ...
    subgraph DI ["Data & Integration"]
        style DI fill:#fffaf0,stroke:#d4a017
        CIL["ChromaDB:<br/>Ingested Laws"]
        PCD["Private ChromaDB:<br/>User Docs"]
        MF["Mapping Files"]
        IK["Indian Kanoon API<br/>(Live Cases/Laws)"]
    end
    ...
    IC -- "Sections" --> DE
    RD -- "Query / Chat" --> ALA
    CP -- "Fetch Precedents" --> IK

    ALA -- "Fetch Precedents" --> IK
    PR --> CIL
    RTS --> PCD
    HRS --> CIL
    HRS -- " " --> PCD
    ...
    style DW fill:#e1f5fe,stroke:#01579b
    style CP fill:#e1f5fe,stroke:#01579b
    
    style ALA fill:#fff3e0,stroke:#e65100
    ...
    style RTS fill:#fff,stroke:#333
    style PR fill:#fff3e0,stroke:#e65100
    
    style CIL fill:#e8f5e9,stroke:#2e7d32
    style PCD fill:#e8f5e9,stroke:#2e7d32
    style IK fill:#e8f5e9,stroke:#2e7d32
